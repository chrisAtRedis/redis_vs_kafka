...
'''
Parse stream functions
'''

def _parse_stream_read_range_helper(tid_range):
    res = []
    if tid_range:
        for i in tid_range:
            time_range, dict_content = i
            res.append((time_range.decode(), pairs_to_dict(dict_content) or {}))
    return res

def parse_stream_range(response, **options):
    return _parse_stream_read_range_helper(response)

def parse_stream_read(response, **options):
    res = []
    if response:
        for i in response:
            stream, rest = i
            res.append((stream.decode(), _parse_stream_read_range_helper(rest)))
    return res

def parse_stream_id(response, **options):
    return response.decode()

# TO BE VERIFIED!
def parse_stream_pending(response, **options):
    res = []
    for streams in response:
        pass
...
class StrictRedis(object):
    ...
    RESPONSE_CALLBACKS = dict_merge(
        ...
        string_keys_to_dict('XADD', parse_stream_id),
        string_keys_to_dict('XPENDING', parse_stream_pending),
        string_keys_to_dict('XRANGE XREVRANGE', parse_stream_range),
        string_keys_to_dict('XREAD XREADGROUP', parse_stream_read),
    ...
    # Helper function: return correctly formatted maxlen string
    def __max_len_helper(self, maxlen, approx):
        m_str = 'MAXLEN '
        m_str += '~ ' + str(maxlen) if approx else str(maxlen)
        return m_str

    def xadd(self, stream, mapping, id='*', maxlen=None, approx=False):
        items = []
        for pair in iteritems(mapping):
            items.extend(pair)

        if maxlen is not None:
            m_str = self.__max_len_helper(maxlen, approx)
            return self.execute_command('XADD', stream, m_str.split(), id, *items)
        return self.execute_command('XADD', stream, id, *items)

    def xlen(self, stream):
        return self.execute_command('XLEN', stream)

    def xpending(self, stream, group, start=None, stop=None, count=None, consumer=None):
        # Either all of start, end and count are set or none
        f_1 = bool(start) + bool(stop) + bool(count)
        if not f_1 in [0, 3]:
            raise RedisError('XPENDING requires all of start, stop and count or none')
        if f_1 == 3 and consumer == None:
            return self.execute_command('XPENDING', stream, group, start, stop, count)
        elif f_1 == 3 and not consumer == None:
            return self.execute_command('XPENDING', stream, group, start, stop, count, consumer)
        return self.execute_command('XPENDING', stream, group)

    def xrange(self, stream, start, end, count=None, cmd='XRANGE'):
        if count is not None:
            return self.execute_command(cmd, stream, start, end, 'COUNT', count)
        return self.execute_command(cmd, stream, start, end)

    def xrevrange(self, stream, start, end, count=None):
        return self.xrange(stream, start, end, count, 'XREVRANGE')

    def xread(self, streams, ids, count=None, block=None):
        if len(streams) == 0:
            raise RedisError('XREAD requires at least one stream')
        if not len(ids) == 0:
            raise RedisError('XREAD requires at least one id')
        c_str = ''
        b_str = ''
        if count is not None:
            c_str = 'COUNT ' + str(count)
        if block is not None:
            b_str = 'BLOCK ' + str(block)

        if count is not None and block is not None:
            return self.execute_command('XREAD', *c_str.split(), *b_str.split(), 'STREAMS', *streams, *ids)
        elif count is not None:
            return self.execute_command('XREAD', *c_str.split(), 'STREAMS', *streams, *ids)
        elif block is not None:
            return self.execute_command('XREAD', *b_str.split(), 'STREAMS', *streams, *ids)
        return self.execute_command('XREAD', 'STREAMS', *streams, *ids)

    def xdel(self, stream, ids):
        if ids:
            return self.execute_command('XDEL', stream, *ids)
        else:
            raise RedisError('XDEL requires at least one ID')

    def xtrim(self, stream, maxlen, approx=None):
        m_str = 'MAXLEN ' + str(maxlen)
        if approx:
            m_str = '~ ' + m_str
        return self.execute_command('XTRIM', stream, *m_str.split())

    def xgroup(self, sub_cmd, stream, group, *args):
        if sub_cmd.lower() not in ['create', 'setid', 'destroy', 'delconsumer']:
            raise(RedisError('XGROUP: ' + sub_cmd + 'not a valid subocmmand'))
        return self.execute_command('XGROUP', sub_cmd.upper(), stream, group, *args)

    def xreadgroup(self, group, consumer, streams, ids, count=None, block=None):
        if not streams:
            raise RedisError('XREADGROUP: requires at least one stream')
        if not ids:
            raise RedisError('XREADGROUP: requires at least one id')
        c_str = ''
        b_str = ''
        if count is not None:
            c_str = 'COUNT ' + str(count)
        ...
        if count is not None and block is not None:
            return self.execute_command('XREADGROUP', 'GROUP', group, consumer,
                                        *c_str.split(), *b_str.split(), 'STREAMS', *streams, *ids)
        elif count is not None:
            return self.execute_command('XREADGROUP', 'GROUP', group, consumer,
                                        *c_str.split(), 'STREAMS', *streams, *ids)
        elif block is not None:
            return self.execute_command('XREADGROUP', 'GROUP', group, consumer,
                                        *b_str.split(), 'STREAMS', *streams, *ids)

        return self.execute_command('XREADGROUP', 'GROUP', group, consumer, 'STREAMS', *streams, *ids)

    def xack(self, stream, group, *ids):
        if not ids:
            raise RedisError('XACK: requires at least one id')
        return self.execute_command('XACK', stream, group, *ids)
